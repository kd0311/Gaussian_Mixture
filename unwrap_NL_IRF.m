function [ Sigma] = unwrap_NL_IRF( params,epsilon_vec,setup)
...
Sigma=zeros(setup.size_obs,setup.size_obs,setup.lags);
...
for jj=1:setup.lags
   ...
   Sigma(:,oo,jj)=sum(SL_NL_3(params.beta_gen_neg(:,:,oo),params.b_gen_neg(:,:,oo),params.c_gen_neg(:,:,oo),params.beta_gen_pos(:,:,oo),params.b_gen_pos(:,:,oo),params.c_gen_pos(:,:,oo),epsilon_vec(setup.index_unrestricted,jj),jj,setup.size_obs),2);
